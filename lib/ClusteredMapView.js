...
const ClusteredMapView = forwardRef(
  (
    ...
    const [markers, updateMarkers] = useStateWithCallbackLazy([]);
    ...
    const _getParent = (fromMarkers, coordinates) => {
      const clusters = fromMarkers.filter(m => m.properties.cluster);
      // console.log(`clusters: ${JSON.stringify(_simplifyMarkers(clusters))}`);
      for (const cluster of clusters) {
        const cIndex = superCluster.getChildren(cluster.id).findIndex(({geometry: {coordinates: childCoord}}) =>
          childCoord[0] === coordinates[0] && childCoord[1] === coordinates[1]
        );
        if (cIndex >= 0) {
          return cluster;
        }
    ...
    const _divideAndMerge = (_markers, updateMarkers) => {
      let divided = false;

      const dividedMarkers = _markers.map((oldMarker) => {
        const {geometry: {coordinates}} = oldMarker;
        const parent = _getParent(markers, coordinates);

        if (!!parent) {
          const {geometry: {coordinates: parentCoord}} = parent;
          oldMarker.geometry.newCoordinates = coordinates;
          oldMarker.geometry.coordinates = parentCoord;
          divided = true;
        }
      ...
      if (divided) {
        updateMarkers(dividedMarkers, updatedMarkers => {
          updatedMarkers.filter(m => m.geometry.newCoordinates?.length).forEach(newMarker => {
            newMarker.geometry.coordinates = [...newMarker.geometry.newCoordinates];
          });
          updateMarkers([...updatedMarkers]);
        });
      } else {
        const mergedMarkers = markers.map((oldMarker, newIndex) => {
          const {geometry: {coordinates}} = oldMarker;
          const parent = _getParent(_markers, coordinates);
        ...
        updateMarkers(mergedMarkers, () => {
          setTimeout(() => {
            updateMarkers(_markers);
          }, animationEnabled ? 500 : 0);
        });
      }
    };
    ...
    const _onRegionChangeComplete = (region) => {
      if (superCluster && region) {
        ...
        _divideAndMerge(markers, updateMarkers);
